...
class TestProtectedEndpoints:
    ...
    def test_public_data_endpoints_accessible(self, client, method, path):
        """Data listing endpoints require authentication (post-F1 hardening).
        ...
        with patch(
            "api.routers.unidades_proyecto.get_firestore_client", return_value=mock_db
        ), patch("database.firebase_config.get_firestore_client", return_value=mock_db):
            resp = getattr(client, method)(path)

        # Endpoints ahora protegidos: 401 sin auth; 200/500 si la dep auth se bypasea en tests futuros.
        assert resp.status_code in [
            200,
            401,
            403,
...
class TestGlobalExceptionHandler:
    ...
    def test_unhandled_error_returns_json(self, client):
        ...
        handler = app.exception_handlers.get(Exception) or app.exception_handlers.get(
            500
        )
