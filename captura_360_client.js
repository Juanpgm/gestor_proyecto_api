...
class Captura360Client {
  ...
  async crearCaptura(params) {
    ...
    if (!upid || !nombre_up || !estado_360 || !photosUrl.length) {
      throw new Error(
        "Parámetros requeridos faltantes: upid, nombre_up, estado_360, photosUrl",
      );
    }
    ...
    if (!estadosValidos.includes(estado_360)) {
      throw new Error(
        `estado_360 inválido. Debe ser uno de: ${estadosValidos.join(", ")}`,
      );
    }
    ...
    if (urlsInvalidas.length > 0) {
      throw new Error(
        `URLs inválidas encontradas: ${urlsInvalidas.join(", ")}`,
      );
    }
    ...
    if (!tiposValidos.includes(tipo_visita)) {
      throw new Error(
        `tipo_visita inválido. Debe ser uno de: ${tiposValidos.join(", ")}`,
      );
    }
  ...
  async obtenerRegistros(filtros = {}) {
    ...
    if (filtros.nombre_centro_gestor)
      params.append("nombre_centro_gestor", filtros.nombre_centro_gestor);
