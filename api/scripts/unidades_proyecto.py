...
def _convert_to_int(value) -> Optional[int]:
    ...
    if (
        value is None
        or value == ""
        or str(value).strip() in ["null", "None", "nan", "NaN"]
    ):
        return None
    try:
...
def crear_intervencion_desde_documento(
    doc_data: Dict[str, Any], index: int = 0
) -> Dict[str, Any]:
    """
...
def transformar_documento_a_unidad_con_intervenciones(
...
def aplicar_filtros_a_intervenciones(
    ...
    for feature in features:
        ...
        for interv in intervenciones:
            ...
            if (
                tipo_intervencion
                and interv.get("tipo_intervencion") != tipo_intervencion
            ):
                cumple = False
...
def apply_client_side_filters(
    data: List[Dict[str, Any]], filters: Optional[Dict[str, Any]] = None
) -> List[Dict[str, Any]]:
    """
    ...
    try:
        ...
        if "upid" in filters and filters["upid"]:
            ...
            if isinstance(upid_filter, list):
                filtered_data = [
                    ...
                    if item.get("upid") in upid_filter
                    or item.get("properties", {}).get("upid") in upid_filter
                ]
            else:
                filtered_data = [
                    item
                    for item in filtered_data
                    if item.get("upid") == upid_filter
                    or item.get("properties", {}).get("upid") == upid_filter
                ]
        ...
        if "estado" in filters and filters["estado"]:
            ...
            def tiene_estado(item):
                ...
                if (
                    item.get("estado") == estado_value
                    or item.get("properties", {}).get("estado") == estado_value
                ):
                    return True
                # Nivel de intervenciones (estructura nueva)
                intervenciones = item.get("intervenciones") or item.get(
                    "properties", {}
                ).get("intervenciones", [])
        ...
        if "tipo_intervencion" in filters and filters["tipo_intervencion"]:
            ...
            def tiene_tipo_intervencion(item):
                # Nivel directo (estructura antigua)
                if (
                    item.get("tipo_intervencion") == tipo_value
                    or item.get("properties", {}).get("tipo_intervencion") == tipo_value
                ):
                    return True
                # Nivel de intervenciones (estructura nueva)
                intervenciones = item.get("intervenciones") or item.get(
            ...
            filtered_data = [
                item for item in filtered_data if tiene_tipo_intervencion(item)
            ]
        ...
        if "clase_up" in filters and filters["clase_up"]:
            ...
            filtered_data = [
                item
                for item in filtered_data
                if item.get("clase_up") == clase_value
                or item.get("clase_obra") == clase_value
                or item.get("properties", {}).get("clase_up") == clase_value
                or item.get("properties", {}).get("clase_obra") == clase_value
            ]
        ...
        if "tipo_equipamiento" in filters and filters["tipo_equipamiento"]:
            ...
            filtered_data = [
                item
                for item in filtered_data
                if item.get("tipo_equipamiento") == tipo_equip_value
                or item.get("properties", {}).get("tipo_equipamiento")
                == tipo_equip_value
            ]
        ...
        if "nombre_centro_gestor" in filters and filters["nombre_centro_gestor"]:
            ...
            filtered_data = [
                item
                for item in filtered_data
                if item.get("nombre_centro_gestor") == centro_value
                or item.get("properties", {}).get("nombre_centro_gestor")
                == centro_value
            ]
        ...
        if "departamento" in filters and filters["departamento"]:
            ...
            filtered_data = [
                item
                for item in filtered_data
                if item.get("departamento") == dept_value
                or item.get("properties", {}).get("departamento") == dept_value
            ]
        ...
        if "municipio" in filters and filters["municipio"]:
            ...
            filtered_data = [
                item
                for item in filtered_data
                if item.get("municipio") == mun_value
                or item.get("properties", {}).get("municipio") == mun_value
            ]
        ...
        if "comuna_corregimiento" in filters and filters["comuna_corregimiento"]:
            ...
            filtered_data = [
                item
                for item in filtered_data
                if item.get("comuna_corregimiento") == comuna_value
                or item.get("properties", {}).get("comuna_corregimiento")
                == comuna_value
            ]
        ...
        if "barrio_vereda" in filters and filters["barrio_vereda"]:
            ...
            filtered_data = [
                item
                for item in filtered_data
                if item.get("barrio_vereda") == barrio_value
                or item.get("properties", {}).get("barrio_vereda") == barrio_value
            ]
        ...
        if "frente_activo" in filters and filters["frente_activo"]:
            ...
            def tiene_frente_activo(item):
                # Nivel directo (estructura antigua)
                if (
                    item.get("frente_activo") == frente_value
                    or item.get("properties", {}).get("frente_activo") == frente_value
                ):
                    return True
                # Nivel de intervenciones (estructura nueva)
                intervenciones = item.get("intervenciones") or item.get(
            ...
            filtered_data = [
                item for item in filtered_data if tiene_frente_activo(item)
            ]
        ...
        if "presupuesto_base" in filters and filters["presupuesto_base"]:
            try:
                ...
                filtered_data = [
                    ...
                    for item in filtered_data
                    if (
                        item.get("presupuesto_base")
                        and float(item["presupuesto_base"]) >= min_presupuesto
                    )
                    or (
                        item.get("properties", {}).get("presupuesto_base")
                        and float(item["properties"]["presupuesto_base"])
                        >= min_presupuesto
                    )
                ]
            except (ValueError, TypeError):
        ...
        if "avance_obra" in filters and filters["avance_obra"]:
            try:
                ...
                filtered_data = [
                    ...
                    for item in filtered_data
                    if (
                        item.get("avance_obra")
                        and float(item["avance_obra"]) >= min_avance
                    )
                    or (
                        item.get("properties", {}).get("avance_obra")
                        and float(item["properties"]["avance_obra"]) >= min_avance
                    )
                ]
            except (ValueError, TypeError):
                pass
        ...
        if "search" in filters and filters["search"]:
            ...
            filtered_data = [
                item for item in filtered_data if search_in_record(item, search_term)
            ]
...
def search_in_record(record: Dict[str, Any], search_term: str) -> bool:
    ...
    for field in searchable_fields:
        if (
            field in record
            and record[field]
            and search_term in str(record[field]).lower()
        ):
            return True
    ...
    for field in searchable_fields:
        if (
            ...
            and search_term in str(properties[field]).lower()
        ):
            return True
...
def is_point_in_bbox(
    ...
    try:
        ...
        lat = (
            ...
            or record.get("properties", {}).get("lat")
        )
        lng = (
            record.get("lng")
            or record.get("longitude")
            or record.get("properties", {}).get("lng")
        )
...
def check_date_filter(
    record: Dict[str, Any], filter_type: str, date_value: str
) -> bool:
...
async def get_all_unidades_proyecto_simple(
    ...
    try:
        ...
        logger.error(
            f"Error en get_all_unidades_proyecto_simple: {str(e)}\n{traceback.format_exc()}"
        )
...
def extraer_geometria_exhaustiva(
    doc_data: Dict[str, Any], upid: str, debug: bool = False
) -> tuple:
    """
            ...
            if isinstance(geo_value, dict) and "type" in geo_value:
                ...
                if geom_type in valid_types:
                    ...
                    if geom_type == "GeometryCollection":
                        if (
                            "geometries" in geo_value
                            and len(geo_value["geometries"]) > 0
                        ):
                            if debug:
                                logger.debug(
                                    f"Geometria encontrada en: {geo_name} (GeometryCollection)"
                                )
                    ...
                    # Para otros tipos, validar que tenga coordinates
                        ...
                        if coords and coords != [0, 0] and coords != [None, None]:
                            if debug:
                                logger.debug(
                                    f"Geometria encontrada en: {geo_name} ({geom_type})"
                                )
                            return geo_value, True, geo_name
                        elif debug:
                            logger.debug(
                                f"{geo_name} tiene coordenadas nulas: {coords}"
                            )
        except Exception as e:
            if debug:
    ...
    if lat is not None and lng is not None:
        if -90 <= lat <= 90 and -180 <= lng <= 180:
            geometry_obj = {"type": "Point", "coordinates": [lng, lat]}
    ...
    try:
        ...
        for doc in docs:
            ...
            if total_docs_processed <= 3:
                logger.debug(
                    f"Doc {total_docs_processed}: {doc.id} keys={list(doc_data.keys())[:10]}"
                )
            ...
            upid_value = (
                record.get("upid")
                or doc_data.get("upid")
                or doc_data.get("properties", {}).get("upid")
            )
            ...
            if upid_value:
                ...
                geometry_data_obj, geometry_found, geometry_source = (
                    extraer_geometria_exhaustiva(doc_data, upid_value, debug=debug_mode)
                )
                ...
                if not geometry_found or not geometry_data_obj:
                    ...
                    if debug_mode:
                        logger.debug(
                            f"Usando coordenadas placeholder [0,0] para {upid_value}"
                        )
                elif debug_mode:
                ...
                if "intervenciones" in doc_data and isinstance(
                    doc_data.get("intervenciones"), list
                ):
                    ...
                    clase_up_val = doc_data.get("clase_up") or doc_data.get(
                        "clase_obra"
                    )
                    ...
                    unidad_properties = {
                        ...
                        "comuna_corregimiento": doc_data.get("comuna_corregimiento"),
                        "comuna_corregimiento_2": doc_data.get(
                            "comuna_corregimiento_2"
                        ),
                        "tipo_equipamiento": tipo_equipamiento_val,
                        "clase_up": clase_up_val,
                        "nombre_centro_gestor": doc_data.get("nombre_centro_gestor"),
                        ...
                        "n_intervenciones": doc_data.get(
                            "n_intervenciones", len(intervenciones_parsed)
                        ),
        ...
        logger.debug(
            f"Procesados {total_docs_processed} docs, incluidos {len(geometry_data)} registros totales"
        )
        ...
        if filters:
            ...
            if "upid" in filters and filters["upid"]:
                ...
                logger.debug(
                    f"Filtro por upid aplicado: {len(geometry_data)} registros"
                )
            ...
            if filtros_intervencion:
                ...
                logger.debug(
                    f"Filtros de intervencion aplicados: {len(geometry_data)} registros"
                )
            ...
            if filtros_unidad:
                ...
                logger.debug(
                    f"Filtros de unidad aplicados: {len(geometry_data)} registros"
                )
        ...
        logger.error(
            f"Error en get_unidades_proyecto_geometry: {str(e)}\n{traceback.format_exc()}"
        )
...
async def get_unidades_proyecto_attributes(
    ...
    try:
        ...
        logger.debug(
            f"get_unidades_proyecto_attributes - Filtros detectados: {has_filters}"
        )
        ...
        if (
            ...
            and "upid" in filters
            and filters["upid"]
            and not isinstance(filters["upid"], list)
        ):
        ...
        if limit and limit > 0:
            ...
            logger.debug(
                f"SERVER-SIDE limite pospuesto para aplicar despues de filtros: {limit}"
            )
        ...
        for doc in docs:
                ...
                for field, value in doc_data["properties"].items():
                    if field not in geometry_fields and field not in attributes_record:
                        ...
                        if field == "presupuesto_base":
                            ...
                            attributes_record[field] = _convert_bpin_to_positive_int(
                                value
                            )
                        else:
                            attributes_record[field] = value
            ...
            if "intervenciones" in attributes_record and isinstance(
                attributes_record["intervenciones"], list
            ):
                import json
                ...
                # Enriquecer cada intervención con estado calculado y frente_activo
                clase_up_attr = attributes_record.get(
                    "clase_up"
                ) or attributes_record.get("clase_obra")
                tipo_equip_attr = attributes_record.get("tipo_equipamiento")
                unidad_props_attr = {
                    "clase_up": clase_up_attr,
                    "tipo_equipamiento": tipo_equip_attr,
                }
                intervenciones_parsed = [
        ...
        if filters:
            ...
            if "nombre_centro_gestor" in filters and filters["nombre_centro_gestor"]:
                client_side_filters["nombre_centro_gestor"] = filters[
                    "nombre_centro_gestor"
                ]
            ...
            if "comuna_corregimiento" in filters and filters["comuna_corregimiento"]:
                client_side_filters["comuna_corregimiento"] = filters[
                    "comuna_corregimiento"
                ]
            ...
            if client_side_filters:
                logger.debug(
                    f"CLIENT-SIDE filtros aplicados: {client_side_filters_applied}"
                )
                attributes_data = apply_client_side_filters(
                    attributes_data, client_side_filters
                )
                logger.debug(
                    f"RESULTADO FINAL - Registros despues de filtros: {len(attributes_data)} de {total_docs} descargados"
                )
        ...
        optimization_info = (
            "Con filtros aplicados" if has_filters else "Sin filtros - Datos completos"
        )
        ...
        logger.error(
            f"Error en get_unidades_proyecto_attributes: {str(e)}\n{traceback.format_exc()}"
        )
...
async def get_unidades_proyecto_dashboard(
    filters: Optional[Dict[str, Any]] = None,
) -> Dict[str, Any]:
    """
    ...
    try:
        ...
        attributes_result = await get_unidades_proyecto_attributes(
            dashboard_filters, limit=None
        )

        # Verificar resultados
        geometry_success = (
            geometry_result.get("type") == "FeatureCollection"
            or geometry_result.get("success") == True
        )
        ...
        for record in all_records:
            ...
            presupuesto_raw = record.get("presupuesto_base") or properties.get(
                "presupuesto_base"
            )
            ...
            fuente = record.get("fuente_financiacion") or properties.get(
                "fuente_financiacion"
            )
            if (
                fuente
                and str(fuente).strip()
                and str(fuente).strip().lower()
                not in ["null", "none", "", "por definir"]
            ):
            ...
            tipo = record.get("tipo_intervencion") or properties.get(
                "tipo_intervencion"
            )
            if tipo and str(tipo).strip():
                tipos_intervencion[tipo] = tipos_intervencion.get(tipo, 0) + 1

            centro = record.get("nombre_centro_gestor") or properties.get(
                "nombre_centro_gestor"
            )
            if centro and str(centro).strip():
                centros_gestores[centro] = centros_gestores.get(centro, 0) + 1

            comuna = record.get("comuna_corregimiento") or properties.get(
                "comuna_corregimiento"
            )
            if comuna and str(comuna).strip():
                comunas_corregimientos[comuna] = (
                    comunas_corregimientos.get(comuna, 0) + 1
                )
            ...
            if lat is None or lng is None:
                ...
                for lat_val in lat_sources:
                    if lat_val is not None and str(lat_val).strip() not in [
                        "",
                        "null",
                        "None",
                    ]:
                ...
                for lng_val in lng_sources:
                    if lng_val is not None and str(lng_val).strip() not in [
                        "",
                        "null",
                        "None",
                    ]:
            ...
            if lat is not None and lng is not None:
                try:
                    ...
                    if -10 <= lat_float <= 20 and -90 <= lng_float <= -60:
                        ...
                        if len(latitudes) <= 3:
                            logger.debug(
                                f"Coordenada {len(latitudes)} - Lat: {lat_float}, Lng: {lng_float}"
                            )
                except Exception as e:
                    pass
            ...
            for campo in campos_criticos:
                ...
                if (
                    valor is not None
                    and str(valor).strip()
                    and str(valor).strip().lower() not in ["null", "none", ""]
                ):
        ...
        if presupuestos_validos:
            ...
            metricas_financieras = {
                "resumen": {
                    ...
                    "presupuesto_promedio": round(
                        total_presupuesto / len(presupuestos_validos), 2
                    ),
                    "presupuesto_mediano": presupuestos_ordenados[
                        len(presupuestos_ordenados) // 2
                    ],
                ...
                "distribucion_rangos": {
                    "menos_100M": len(
                        [p for p in presupuestos_validos if p < 100_000_000]
                    ),
                    "100M_1B": len(
                        [
                            p
                            for p in presupuestos_validos
                            if 100_000_000 <= p < 1_000_000_000
                        ]
                    ),
                    "1B_10B": len(
                        [
                            p
                            for p in presupuestos_validos
                            if 1_000_000_000 <= p < 10_000_000_000
                        ]
                    ),
                    "mas_10B": len(
                        [p for p in presupuestos_validos if p >= 10_000_000_000]
                    ),
                },
                "percentiles": {
                    ...
                    "p90": presupuestos_ordenados[
                        9 * len(presupuestos_ordenados) // 10
                    ],
                },
            }
        ...
        if avances_validos:
            ...
            metricas_avance = {
                "resumen": {
                    ...
                    "avance_mediano": sorted(avances_validos)[
                        len(avances_validos) // 2
                    ],
                },
                "distribucion_avance": {
        ...
        def crear_distribucion_grafico(datos_dict, max_items=15, incluir_otros=True):
            ...
            items_ordenados = sorted(
                datos_dict.items(), key=lambda x: x[1], reverse=True
            )

            # Tomar los top items
            top_items = items_ordenados[:max_items]
            otros_count = (
                sum(v for k, v in items_ordenados[max_items:])
                if len(items_ordenados) > max_items
                else 0
            )
        ...
        logger.debug(
            f"Frentes de obra activos calculados: {frentes_activos_count} en {unidades_con_frentes_count} unidades"
        )
        ...
        dashboard_data = {
            ...
            "resumen_ejecutivo": {
                ...
                "presupuesto_total_formateado": (
                    f"${sum(presupuestos_validos):,.0f}"
                    if presupuestos_validos
                    else "N/D"
                ),
                "avance_promedio": (
                    round(sum(avances_validos) / len(avances_validos), 1)
                    if avances_validos
                    else 0
                ),
            ...
            "distribuciones_graficos": {
                ...
                "tipos_intervencion": crear_distribucion_grafico(
                    tipos_intervencion, 12
                ),
                "centros_gestores": crear_distribucion_grafico(centros_gestores, 15),
                "comunas_corregimientos": crear_distribucion_grafico(
                    comunas_corregimientos, 20
                ),
                "fuentes_financiacion": crear_distribucion_grafico(
                    fuentes_financiacion, 10
                ),
            ...
            "kpis_negocio": {
                "eficiencia_ejecucion": (
                    round(
                        len([a for a in avances_validos if a > 50])
                        / len(avances_validos)
                        * 100,
                        1,
                    )
                ...
                "inversion_promedio_por_comuna": (
                    round(sum(presupuestos_validos) / len(comunas_corregimientos), 0)
                    if presupuestos_validos and comunas_corregimientos
                    else 0
                ),
                "diversidad_tipos": len(tipos_intervencion),
                "cobertura_geografica": (
                    round(len(latitudes) / total_records * 100, 1)
                    if total_records > 0
                    else 0
                ),
                "densidad_proyectos_territorial": (
                    round(total_records / len(comunas_corregimientos), 1)
                    if comunas_corregimientos
                    else 0
                ),
            },
            # 📋 CALIDAD DE DATOS
            "calidad_datos": {
                campo: {
                    ...
                    "calidad_nivel": (
                        "Excelente"
                        if count / total_records >= 0.95
                        else (
                            "Buena"
                            if count / total_records >= 0.80
                            else (
                                "Regular"
                                if count / total_records >= 0.60
                                else "Deficiente"
                            )
                        )
                    ),
            ...
            dashboard_data["analisis_geografico"] = {
                "cobertura": {
                    ...
                    "cobertura_porcentaje": round(
                        (len(latitudes) / total_records) * 100, 1
                    ),
                },
                "centro_gravedad": {
                ...
                "bounding_box": {
                    ...
                    "sur": min(latitudes),
                    "este": max(longitudes),
                    "oeste": min(longitudes),
                    "area_km2": round(
                        abs(max(latitudes) - min(latitudes))
                        * abs(max(longitudes) - min(longitudes))
                        * 111.32**2,
                        2,
                    ),
                },
                "densidad_geografica": round(
                    len(latitudes)
                    / max(
                        1,
                        abs(max(latitudes) - min(latitudes))
        ...
        logger.debug(
            f"Procesamiento completado - Coords: {len(latitudes)}, financieros: {len(presupuestos_validos)}, avances: {len(avances_validos)}"
        )
        ...
        logger.error(
            f"Error en get_unidades_proyecto_dashboard: {str(e)}\n{traceback.format_exc()}"
        )
                ...
                if interv.get("intervencion_id") == intervencion_id:
                    return {
                        "unidad": {
                            ...
                            "comuna_corregimiento": properties.get(
                                "comuna_corregimiento"
                            ),
        ...
        return {
            ...
            "properties": {
                ...
                "total_intervenciones": sum(
                    len(f["properties"]["intervenciones"]) for f in features_filtradas
                ),
                "filters": {
                    "estado": estado,
                    "tipo_intervencion": tipo_intervencion,
...
def _clasificar_frente_activo(
    intervencion: Dict[str, Any], unidad_props: Dict[str, Any]
) -> str:
    """
    ...
    condiciones_base = (
        clase_up in clases_validas
        and tipo_equipamiento not in tipos_equipamiento_excluidos
        and tipo_intervencion not in tipos_intervencion_excluidos
    )
    ...
    try:
        ...
        for doc in db.collection("unidades_proyecto").stream():
            ...
            unidades_por_upid[upid] = {
                "clase_up": doc_data.get("clase_up")
                or doc_data.get("clase_obra")
...
async def get_unidades_proyecto_summary() -> Dict[str, Any]:
    ...
    try:
        ...
        if not data:
            return {
                "success": True,
                "summary": {"total": 0, "message": "No hay datos disponibles"},
            }
        ...
        with_geometry = sum(
            1 for item in data if item.get("geometry") or item.get("coordinates")
        )
...
async def validate_unidades_proyecto_collection() -> Dict[str, Any]:
    ...
    try:
        db = get_firestore_client()
        if db is None:
            return {"valid": False, "error": "No se pudo conectar a Firestore"}
        ...
        return {"valid": False, "error": f"Error validando colección: {str(e)}"}
...
async def get_filter_options(
    field: Optional[str] = None, limit: Optional[int] = None
) -> Dict[str, Any]:
    """
    ...
    try:
        ...
        for record in attributes_data:
            ...
            for field_key, field_path in available_fields.items():
                ...
                if (
                    value
                    and str(value).strip()
                    and str(value).strip().lower() not in ["null", "none", ""]
                ):
        ...
        all_filters = {
            field_key: sorted(list(values_set))
    ...
    try:
        ...
        if filters:
            ...
            if filters.get("nombre_centro_gestor"):
                query = query.where(
                    "nombre_centro_gestor", "==", filters["nombre_centro_gestor"]
                )
        ...
        logger.error(
            f"Error obteniendo control de calidad: {str(e)}\n{traceback.format_exc()}"
        )
        return {
            "success": False,
            "error": str(e),
