...
def _generate_presigned_url(bucket: str, s3_key: str) -> Optional[str]:
    ...
    try:
        credentials_path = (
            os.getenv("AWS_CREDENTIALS_FILE_REPORTES_CONTRATOS")
            or os.getenv("AWS_CREDENTIALS_FILE_UNIDADES_PROYECTO")
            or ""
        )
        ...
        logger.warning(
            f"No se pudo generar presigned URL para s3://{bucket}/{s3_key}: {e}"
        )
        return None
...
def _normalize_archivos_for_frontend(doc_data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    for idx, archivo in enumerate(archivos):
        ...
        if (not bucket or not s3_key) and url_publica_directa:
            parsed_bucket, parsed_key = _extract_bucket_key_from_url(
                url_publica_directa
            )
        ...
        archivo_normalizado = {
            ...
            or _detect_content_type(archivo.get("name") or "archivo"),
            "bucket": bucket,
            "s3_key": s3_key,
    ...
    doc_data["links"] = {
        ...
        "visores": {
            "imagenes": doc_data["imagenes_urls"],
            "documentos_inline": [
                u for u in doc_data["documentos_urls"] if u.lower().endswith(".pdf")
            ],
            "documentos_download": [
                u for u in doc_data["documentos_urls"] if not u.lower().endswith(".pdf")
            ],
        },
    }
...
def upload_files_to_s3(
    referencia_contrato: str, archivos: List[Dict[str, Any]]
    ...
    credentials_path = os.getenv(
        "AWS_CREDENTIALS_FILE_REPORTES_CONTRATOS"
    ) or os.getenv("AWS_CREDENTIALS_FILE_UNIDADES_PROYECTO")
    ...
    fotos_prefix = os.getenv(
        "S3_PREFIX_REPORTES_CONTRATOS_FOTOS", "reportes_contratos_fotos"
    ).strip("/")
    docs_prefix = os.getenv(
        "S3_PREFIX_REPORTES_CONTRATOS_DOCUMENTOS", "reportes_contratos_documentos"
    ).strip("/")
    ...
    for idx, archivo in enumerate(archivos):
        ...
        content_type = _detect_content_type(
            nombre_original, archivo.get("content_type", "")
        )
        ...
        try:
            ...
            safe_name = _sanitize_for_s3(
                os.path.splitext(nombre_original)[0], default=f"archivo_{idx + 1}"
            )
            ...
            content_disposition = (
                "inline"
                if (is_image or ext_lower == ".pdf")
                else f'attachment; filename="{object_name}"'
            )
            s3_client.put_object(
                ...
                Metadata={
                    "referencia_contrato": _sanitize_for_s3(
                        referencia_contrato, "sin_ref"
                    )[:120],
                    "tipo": "imagen" if is_image else "documento",
                    "original_filename": _sanitize_for_s3(nombre_original, "archivo")[
                        :150
                    ],
            ...
            failed.append({"name": nombre_original, "error": str(upload_error)})
    ...
    for key, value in doc_data.items():
        ...
        if hasattr(value, "isoformat") and callable(getattr(value, "isoformat")):
            ...
            for item in value:
                ...
                elif hasattr(item, "isoformat") and callable(
                    getattr(item, "isoformat")
                ):
...
def create_drive_folder(
    ...
    base_url = (
        diagnostico["buckets"]["documentos"] if diagnostico.get("buckets") else ""
    )
...
def upload_files_to_folder(
    service, archivos: List[Dict[str, Any]], folder_id: str
) -> List[Dict[str, str]]:
...
def upload_single_file(
    service, archivo: Dict[str, Any], folder_id: str
) -> Dict[str, str]:
...
def validate_uploaded_files(
    archivos_evidencia: List[Dict[str, Any]],
) -> Tuple[bool, str]:
    """
...
async def create_reporte_contrato(reporte_data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        ...
        is_valid, validation_message = validate_uploaded_files(
            reporte_data.get("archivos_evidencia", [])
        )
        if not is_valid:
        ...
        if not nombre_centro_gestor:
            try:
                nombre_centro_gestor_resolved = (
                    await get_nombre_centro_gestor_from_emprestito(
                ...
                logger.warning(
                    f"⚠️ No se pudo auto-resolver nombre_centro_gestor: {resolve_error}"
                )
        ...
        doc_data = {
            ...
            "imagenes_urls": [
                ...
                if a.get("tipo") == "imagen" and a.get("url_publica")
            ],
            "documentos_urls": [
                a.get("url_publica")
                for a in archivos_subidos
                if a.get("tipo") == "documento" and a.get("url_publica")
            ],
...
async def get_nombre_centro_gestor_from_emprestito(
    referencia_contrato: str,
) -> Optional[str]:
    """
    ...
    try:
        ...
        logger.warning(
            f"Error obteniendo nombre_centro_gestor para {referencia_contrato}: {e}"
        )
        return None


async def get_data_from_emprestito(
    referencia_contrato: str,
    ...
    try:
        ...
        if docs:
            ...
            bp = doc_data.get("bp", "")
            if nombre_cg or bp:
                return {"nombre_centro_gestor": nombre_cg, "bp": bp}
        ...
        logger.warning(
            f"Error obteniendo datos de empréstito para {referencia_contrato}: {e}"
        )
        return None
    ...
    try:
        ...
        for doc in docs:
            ...
            if (
                ref_contrato
                and (nombre_cg or bp)
                and ref_contrato not in centro_gestor_map
            ):
                centro_gestor_map[ref_contrato] = {
                    "nombre_centro_gestor": nombre_cg,
                    "bp": bp,
        ...
        logger.info(
            f"✅ Mapa de centros gestores y BP creado con {len(centro_gestor_map)} entradas (contratos + órdenes + convenios)"
        )
...
async def get_reportes_contratos(
        ...
        if filtros:
            if "referencia_contrato" in filtros:
                collection_ref = collection_ref.where(
                    "referencia_contrato", "==", filtros["referencia_contrato"]
                )
            if "estado_reporte" in filtros:
                collection_ref = collection_ref.where(
                    "estado_reporte", "==", filtros["estado_reporte"]
                )

        # Obtener documentos sin ordenar para evitar errores con campos faltantes
        try:
            docs = collection_ref.order_by(
                "fecha_reporte", direction="DESCENDING"
            ).stream()
        except Exception as order_error:
            logger.warning(
                f"Error ordenando por fecha_reporte, obteniendo sin ordenar: {order_error}"
            )
        ...
        for doc in docs:
            try:
                ...
                if doc_data:  # Verificar que el documento no esté vacío
                    ...
                    nombre_centro_gestor_actual = converted_data.get(
                    ...
                    if referencia_contrato:
                        ...
                        if emprestito_data:
                            ...
                            if (
                                not nombre_centro_gestor_actual
                                or nombre_centro_gestor_actual.strip() == ""
                            ):
                                nombre_centro_gestor_emprestito = emprestito_data.get(
                                    "nombre_centro_gestor", ""
                                )
                                if nombre_centro_gestor_emprestito:
                                    converted_data["nombre_centro_gestor"] = (
                                        nombre_centro_gestor_emprestito
                                    )
...
async def get_reporte_contrato_by_id(reporte_id: str) -> Dict[str, Any]:
    ...
    try:
        ...
        if not doc.exists:
            return {"success": False, "error": "Reporte no encontrado", "data": None}
            ...
            if emprestito_data:
                ...
                if (
                    ...
                    if nombre_centro_gestor_emprestito:
                        ...
                        logger.info(
                            f"✅ Actualizado nombre_centro_gestor para reporte {reporte_id}: {nombre_centro_gestor_emprestito}"
                        )
                ...
                if not bp_actual or bp_actual.strip() == "":
                    ...
                    if bp_emprestito:
                        converted_data["bp"] = bp_emprestito
                        logger.info(
                            f"✅ Actualizado bp para reporte {reporte_id}: {bp_emprestito}"
                        )
...
async def delete_reporte_contrato(reporte_id: str) -> Dict[str, Any]:
    """Eliminar un reporte de contrato por su ID de Firestore."""
    try:
        db = get_firestore_client()
        if db is None:
        ...
        doc_ref.delete()
        logger.info(f"Reporte contrato eliminado: {reporte_id}")
        return {
            "success": True,
            "message": f"Reporte {reporte_id} eliminado correctamente",
        }
    except Exception as e:
        logger.error(f"Error eliminando reporte contrato {reporte_id}: {str(e)}")
        return {"success": False, "error": f"Error eliminando reporte: {str(e)}"}
...
async def get_reportes_by_centro_gestor(nombre_centro_gestor: str) -> Dict[str, Any]:
    ...
    try:
        ...
        query = db.collection("reportes_contratos").where(
            "nombre_centro_gestor", "==", nombre_centro_gestor
        )
        try:
            docs = list(
                query.order_by("fecha_reporte", direction="DESCENDING").stream()
            )
        except Exception as order_error:
            logger.warning(
                f"Fallback sin order_by en get_reportes_by_centro_gestor: {order_error}"
            )
            docs = list(query.stream())
        ...
        for doc in docs:
            ...
            converted_data = convert_firebase_timestamps(doc_data)
            converted_data = _normalize_archivos_for_frontend(converted_data)
            reporte_data = {"id": doc.id, **converted_data}
            reportes.append(reporte_data)
        ...
        try:
            ...
            ordenes_docs = (
                db.collection("ordenes_compra_emprestito")
                .where("nombre_centro_gestor", "==", nombre_centro_gestor)
                .stream()
            )
            ...
            for orden_doc in ordenes_docs:
                ...
                ref_contrato = orden_data.get(
                    "numero_orden"
                )  # En órdenes es numero_orden
                if ref_contrato:
                    referencias_emprestito.add(ref_contrato)

            # Obtener referencias desde convenios_transferencias_emprestito
            convenios_docs = (
                db.collection("convenios_transferencias_emprestito")
                .where("nombre_centro_gestor", "==", nombre_centro_gestor)
                .stream()
            )
            ...
            for referencia in referencias_emprestito:
                ref_docs = (
                    db.collection("reportes_contratos")
                ...
                for ref_doc in ref_docs:
                    ...
                    if not ref_doc_data.get("nombre_centro_gestor", "").strip():
                        ...
                        converted_data = _normalize_archivos_for_frontend(
                            converted_data
                        )
                        reporte_data = {
                            "id": ref_doc.id,
                        ...
                        if not any(r["id"] == reporte_data["id"] for r in reportes):
                            reportes.append(reporte_data)
                            logger.info(
                                f"✅ Agregado reporte desde colecciones de empréstito: {referencia}"
                            )

        except Exception as emprestito_error:
            logger.warning(
                f"Error buscando en colecciones de empréstito: {emprestito_error}"
            )
...
async def get_reportes_by_referencia_contrato(
    referencia_contrato: str,
) -> Dict[str, Any]:
    """
    ...
    try:
        ...
        try:
            ...
            logger.warning(
                f"Fallback sin order_by en get_reportes_by_referencia_contrato: {order_error}"
            )
            docs = list(query.stream())
        ...
        for doc in docs:
            ...
            if (
                ...
                nombre_centro_gestor_emprestito = (
                    await get_nombre_centro_gestor_from_emprestito(referencia_contrato)
                )
                if nombre_centro_gestor_emprestito:
                    reporte_data["nombre_centro_gestor"] = (
                        nombre_centro_gestor_emprestito
                    )
                    reporte_data["nombre_centro_gestor_source"] = (
                        "emprestito_collections"
                    )
                    logger.info(
                        f"✅ Actualizado nombre_centro_gestor para {referencia_contrato}: {nombre_centro_gestor_emprestito}"
                    )
