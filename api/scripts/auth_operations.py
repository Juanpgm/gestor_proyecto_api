...
def _resolve_roles(
    firestore_data: Dict[str, Any], custom_claims: Dict[str, Any]
) -> tuple[list[str], str]:
...
async def validate_password_with_firebase_rest(
...
async def initiate_phone_auth(phone_number: str) -> Dict[str, Any]:
    ...
    try:
        ...
        auth_client = get_auth_client()
        try:
            user_record = auth_client.get_user_by_phone_number(
        ...
        if user_exists:
            ...
            firestore_client = get_firestore_client()
            user_doc = (
                firestore_client.collection("users").document(user_record.uid).get()
            )
...
def _persist_user_sync(
    uid: str,
    user_record,
    user_payload: dict,
    user_doc_exists: bool,
    firestore_data: dict,
):
    ...
    try:
            ...
            firestore_client.collection("users").document(uid).set(
                {
                    ...
                    "is_active": True,
                    "auth_providers": [
                        provider.provider_id for provider in user_record.provider_data
                    ],
                },
                merge=True,
            )
        ...
        logger.warning(
            f"auth.validate_session could not persist/sync for uid={uid}: {e}"
        )
...
async def validate_user_session(id_token: str) -> Dict[str, Any]:
    ...
    try:
        ...
        logger.error("verify_id_token timed out after 15s")
        return {
            "valid": False,
            "error": "Tiempo de espera agotado verificando token",
...
async def revoke_user_tokens(uid: str) -> Dict[str, Any]:
    ...
    try:
        ...
        try:
            firestore_client.collection("users").document(uid).update(
                {"tokens_revoked_at": datetime.now(), "updated_at": datetime.now()}
            )
        except Exception as e:
...
async def check_auth_method_availability(
    ...
    try:
        ...
        return {
            ...
            "google_eligible": email
            and validate_email(email).get("can_use_google_auth", False),
        }
