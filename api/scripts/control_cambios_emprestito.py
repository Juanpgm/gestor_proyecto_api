...
try:
    from database.firebase_config import (
        ...
        FIREBASE_AVAILABLE as FIRESTORE_AVAILABLE,
    )
except ImportError:
    try:
        from ..database.firebase_config import (
...
async def registrar_cambio_valor(
    ...
    try:
        ...
        cambio_data = {
            ...
            "diferencia": (
                float(valor_nuevo) - float(valor_anterior)
        ...
        if archivo_soporte and BOTO3_AVAILABLE:
            try:
                ...
                if s3_result.get("success"):
                    ...
                    logger.info(
                        f"✅ Archivo soporte subido a S3: {s3_result.get('s3_key')}"
                    )
                else:
                    logger.warning(
                        f"⚠️ No se pudo subir archivo soporte: {s3_result.get('error')}"
                    )
...
async def _subir_archivo_soporte_s3(
    archivo: UploadFile, tipo_coleccion: str, identificador: str, change_id: str
) -> Dict[str, Any]:
    """
    ...
    try:
        ...
        safe_filename = "".join(
            c for c in archivo.filename if c.isalnum() or c in (" ", ".", "_", "-")
        ).strip()
        safe_identificador = "".join(
            c for c in identificador if c.isalnum() or c in ("-", "_")
        ).strip()
...
async def obtener_historial_cambios(
    ...
    try:
        ...
        return {"success": True, "total_cambios": len(cambios), "cambios": cambios}
