...
class GeometryPoint(BaseModel):
    ...
    coordinates: List[float] = Field(
        ..., description="Coordenadas [lng, lat] o [lng, lat, elevation]"
    )
...
class GeometryLineString(BaseModel):
    ...
    coordinates: List[List[float]] = Field(
        ..., description="Array de coordenadas [[lng, lat], ...]"
    )
...
class GeometryPolygon(BaseModel):
    ...
    coordinates: List[List[List[float]]] = Field(
        ..., description="Array de anillos de coordenadas"
    )
...
class GeometryMultiLineString(BaseModel):
    ...
    type: str = Field(
        default="MultiLineString", description="Tipo de geometría GeoJSON"
    )
    coordinates: List[List[List[float]]] = Field(
        ..., description="Array de LineStrings"
    )
...
class Intervencion(BaseModel):
    ...
    intervencion_id: str = Field(
        ..., description="ID único de la intervención (generado o desde datos)"
    )
    ...
    estado: Optional[str] = Field(
        None, description="Estado: En ejecución, Terminado, etc."
    )
    tipo_intervencion: Optional[str] = Field(None, description="Tipo de intervención")
    frente_activo: Optional[str] = Field(
        None, description="Estado del frente: Frente activo, Inactivo, No aplica"
    )

    # Información financiera
    presupuesto_base: Optional[float] = Field(
        None, description="Presupuesto base de la intervención"
    )
    ...
    # Progreso
    avance_obra: Optional[float] = Field(
        None, description="Porcentaje de avance (0-100)"
    )
    ...
    referencia_contrato: Optional[str] = Field(
        None, description="Referencia del contrato"
    )
    referencia_proceso: Optional[str] = Field(
        None, description="Referencia del proceso"
    )
...
class UnidadProyectoConIntervenciones(BaseModel):
    ...
    nombre_up_detalle: Optional[str] = Field(
        None, description="Detalle adicional del nombre"
    )
    ...
    comuna_corregimiento: Optional[str] = Field(
    ...
    # Intervenciones
    n_intervenciones: int = Field(
        default=0, description="Número de intervenciones en esta unidad"
    )
    intervenciones: List[Intervencion] = Field(
        default=[], description="Array de intervenciones"
    )
    ...
    has_valid_geometry: bool = Field(
        False, description="Indica si las coordenadas son reales"
    )
...
class UnidadProyectoProperties(BaseModel):
    ...
    # Identificadores
    upid: str = Field(
        ..., description="ID único de la unidad de proyecto (UUID o similar)"
    )
    bpin: Optional[str] = Field(
        None, description="Código BPIN del proyecto (puede tener prefijo '-')"
    )
    ...
    # Clasificación
    estado: Optional[str] = Field(
        None, description="Estado del proyecto: Finalizado, En Ejecución, etc."
    )
    tipo_intervencion: Optional[str] = Field(
        None, description="Tipo de intervención del proyecto"
    )
    clase_up: Optional[str] = Field(
        None, description="Clasificación de la unidad de proyecto: Obra Vial, etc."
    )
    tipo_equipamiento: Optional[str] = Field(
        None, description="Tipo de equipamiento del proyecto"
    )
    identificador: Optional[str] = Field(
        None, description="Identificador del tipo: Grupo Operativo, etc."
    )
    ...
    # Información financiera
    presupuesto_base: Optional[float] = Field(
        None, description="Presupuesto base del proyecto"
    )
    fuente_financiacion: Optional[str] = Field(
        None, description="Fuente de financiación"
    )
    ...
    avance_obra: Optional[float] = Field(
        None, description="Porcentaje de avance de la obra (0-100)"
    )
    cantidad: Optional[int] = Field(None, description="Cantidad o volumen de obra")
    ...
    fecha_inicio: Optional[str] = Field(
        None, description="Fecha de inicio (formato ISO o string)"
    )
    fecha_fin: Optional[str] = Field(None, description="Fecha de finalización")

    # Gestión
    nombre_centro_gestor: Optional[str] = Field(
        None, description="Centro gestor del proyecto"
    )
    referencia_contrato: Optional[str] = Field(
        None, description="Referencia del contrato"
    )
    ...
    geometry_type: Optional[str] = Field(
        None, description="Tipo de geometría: Point, LineString, etc."
    )
    has_geometry: bool = Field(False, description="Indica si tiene geometría válida")
    has_valid_geometry: bool = Field(
        False, description="Indica si las coordenadas son reales (no [0,0])"
    )
    centros_gravedad: bool = Field(False, description="Indica si es centro de gravedad")
    geometry_bounds: Optional[Dict[str, Any]] = Field(
        None, description="Bounding box de la geometría"
    )

    # Campos adicionales
    plataforma: Optional[str] = Field(
        None, description="Plataforma o sistema de origen"
    )
    microtio: Optional[str] = Field(None, description="Campo microtio")

    # Timestamps
    created_at: Optional[str] = Field(
        None, description="Fecha de creación del registro"
    )
    processed_timestamp: Optional[str] = Field(
        None, description="Timestamp de procesamiento"
    )
    ...
    def parse_presupuesto(cls, v):
        """Convertir presupuesto a float, manejando strings con formato. Debe ser >= 0."""
        ...
        if value is None:
            return None
        if value < 0:
            raise ValueError("presupuesto_base no puede ser negativo")
        return value

    @validator("avance_obra", pre=True)
    def parse_avance(cls, v):
        """Convertir avance a float (porcentaje 0..100)."""
        if v is None or v == "" or str(v).strip() in ["null", "None", "nan", "NaN"]:
            return None
        try:
            if isinstance(v, str):
        ...
        if value < 0 or value > 100:
            raise ValueError("avance_obra debe estar entre 0 y 100")
        return value
...
class UnidadProyectoGeoJSON(BaseModel):
    ...
    geometry: Optional[
        ...
        None,
        description="Geometría GeoJSON (Point, LineString, Polygon, MultiLineString)",
    )
    properties: UnidadProyectoProperties = Field(
        ..., description="Propiedades de la unidad de proyecto"
    )
...
class UnidadProyectoFirestore(BaseModel):
    ...
    upid: str = Field(
        ..., description="ID único (será el ID del documento en Firestore)"
    )

    # Geometría (almacenada como dict)
    geometry: Optional[Dict[str, Any]] = Field(
        None, description="Geometría GeoJSON serializada"
    )
    geometry_type: Optional[str] = Field(None, description="Tipo de geometría")
    has_geometry: bool = Field(False, description="Indica si tiene geometría")
    has_valid_geometry: bool = Field(
        False, description="Indica si las coordenadas son válidas"
    )
...
class UnidadProyectoCollectionResponse(BaseModel):
    ...
    features: List[UnidadProyectoGeoJSON] = Field(
        default=[], description="Lista de features"
    )
    properties: Dict[str, Any] = Field(
        default={}, description="Metadata de la colección"
    )
