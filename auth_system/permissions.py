...
def _get_permissions_from_collection(user_uid: str, db_client) -> List[str]:
    ...
    try:
        ...
        user_permissions_doc = (
            db_client.collection(permissions_collection_name).document(user_uid).get()
        )
        if user_permissions_doc.exists:
            ...
            collected_permissions.update(
                _normalize_permissions(
                    user_permissions_data.get("granted_permissions", [])
                )
            )
...
def get_user_permissions(
    ...
    try:
        ...
        # Agregar permisos de gobernanza desde la colección permissions (UNA sola vez)
...
def check_scope_access(
    user_data: Dict, resource_centro_gestor: str, permission: str
) -> bool:
    """
